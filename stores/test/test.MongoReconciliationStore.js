...
var MongoReconciliationStore = require('MongoReconciliationStore');
var Mongo = require("Mongo");

var aOptionsMongoReconciliationStore = {
	database   : "test",
	collection : "recontest",
	safewrites : true
};

// assumes vvTest = [ [method, args, err?, response-val],* ]
var fTestReconciliationStore = function(vvTest,fCallback){
	var mrs = new MongoReconciliationStore(aOptionsMongoReconciliationStore);
...
describe("MongoReconciliationStore", function() {

	// wipe out the entire collection before each test
	beforeEach(function(fDone){
		new Mongo(aOptionsMongoReconciliationStore).fRemove(
			aOptionsMongoReconciliationStore.collection,{},function(err){fDone(null);}
		);
	});
